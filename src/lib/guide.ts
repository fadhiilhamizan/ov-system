...
export const GUIDE: GuideSection[] = [
  ...
  },
  {
    ...
    steps: [
      { id: "Kolom 'Evaluasi' di sebelah kanan kolom Tugas berisi pelajaran untuk tugas itu dari Ormawa Visit sebelumnya dan/atau yang sekarang. Isi lewat kolom Evaluasi di pop-up Edit. Saat tugas disalin ke Ormawa Visit baru (lewat 'Salin data dari Ormawa Visit lain' atau Duplikat), evaluasinya ikut tersalin supaya pelajarannya tidak hilang.", en: "The 'Evaluation' column right of the Task column holds the lessons for that task from the previous and/or the current Ormawa Visit. Fill it in through the Evaluation field in the Edit dialog. When the task is copied into a new Ormawa Visit (via 'Copy data from another Ormawa Visit' or Duplicate), its evaluation is copied too so the lessons are not lost." },
      { id: "Klik 'Tambah Tugas', isi judul, divisi, PIC, dan tanggal mulai/selesai. Pilihan PIC menaruh anggota divisi itu di urutan pertama, dikelompokkan Koordinator / Fungsionaris / Intern, lalu sisa anggota Ormawa Visit ini di bawahnya sebagai 'Anggota divisi lain'. Jadi orang yang belum diisi divisinya, atau yang memang membantu divisi lain, tetap bisa dipilih. Kotak pencarian mencari di kedua bagian.", en: "Click 'Add Task', then fill in the title, division, PIC, and start/end dates. The PIC picker lists that division's members first, grouped by Coordinator / Staff / Intern, then the rest of this Ormawa Visit's roster below them under 'Members of other divisions'. So somebody whose division has not been filled in yet, or who is genuinely helping another division, can still be picked. The search box covers both parts." },
      { id: "Saring dengan kolom pencarian, 'Fokus divisi', 'Fokus PIC', dan filter Status di toolbar. Ketiga penyaring itu pakai checkbox, jadi beberapa nilai bisa aktif bersamaan (misalnya divisi LO dan Event sekaligus, atau status Belum dan Overtime sekaligus). Tanpa centang berarti semuanya tampil, dan menunya sengaja tetap terbuka saat mencentang.", en: "Narrow things down with the search box, the division focus picker, the PIC focus picker, and the Status filter. All three use checkboxes, so several values can be active at once (LO and Event divisions together, say, or the To-do and Overtime statuses together). No ticks means everything is shown, and the menu deliberately stays open while ticking." },
      ...
      { id: "Di pemilih 'Ambil dari Super Link', saring dengan kotak pencarian (nama, divisi, atau nama Ormawa Visit) dan pilihan 'Ormawa Visit' (boleh lebih dari satu), lalu urutkan: Ormawa Visit terbaru, Ormawa Visit terlama, Nama A-Z, atau Nama Z-A. Saat diurutkan per Ormawa Visit, hasilnya dikelompokkan dengan judul tiap edisi. Tautan yang sudah dipakai tugas ini diberi tanda centang.", en: "In the 'Pick from Super Link' picker, filter with the search box (name, division or Ormawa Visit title) and the 'Ormawa Visit' filter (several may be ticked), then sort: newest Ormawa Visit, oldest Ormawa Visit, Name A-Z or Name Z-A. When sorted by Ormawa Visit the results are grouped under each edition's title. Links this task already uses are ticked." },
      ...
      { id: "Setiap tautan hasil WAJIB diberi judul dan otomatis tampil di Super Link dengan judul itu, supaya tugas lain bisa menemukannya. Tidak ada lagi pilihan untuk tidak menampilkannya.", en: "Every result link MUST have a title and is shown in Super Link under that title automatically, so other tasks can find it. There is no longer an option to keep it out." },
    ...
    tips: [
      ...
      { id: "Kalau tautan hasil sedang dijadikan referensi oleh tugas lain, formulirnya menulis 'Dirujuk oleh N tugas lain' dan meminta konfirmasi sekali lagi sebelum menghapusnya. Mengubah alamat atau judulnya ikut terlihat di tugas-tugas itu.", en: "When a result link is being used as a reference by other tasks, the form says 'Referenced by N other tasks' and asks once more before removing it. Changing its address or title shows up in those tasks too." },
      { id: "Kalau entri Super Link yang kamu jadikan referensi dihapus pemiliknya, referensimu TIDAK ikut hilang: alamat dan judul terakhirnya tetap disimpan, lalu referensi itu diberi tanda segitiga kuning. Buka Edit dan tekan 'Ganti' untuk memilih penggantinya dari Super Link.", en: "If the Super Link entry you referenced is deleted by its owner, your reference does NOT disappear: its last address and title are kept and the reference gets a yellow warning triangle. Open Edit and press 'Replace' to pick a substitute from Super Link." },
  ...
  },
  {
    ...
    tips: [
      ...
      { id: "Mengganti nama atau nama panggilan anggota ikut menggantinya di PIC tugas, PIC Job Hari-H, PIC prospek, dan koordinator divisi pada Ormawa Visit yang sama. Kalau ada dua anggota dengan nama yang sama, nama itu dibiarkan supaya tugas orang lain tidak ikut berpindah.", en: "Renaming a member, or changing their nickname, also renames them in task PICs, event-day job PICs, prospect PICs and division coordinators in the same Ormawa Visit. If two members share that name, it is left alone so nobody else's tasks move." },
      { id: "Anggota yang dihapus, keluar dari sebuah divisi, atau diubah jadi intern otomatis dilepas dari kursi koordinator divisi itu. Namanya tetap tercatat di tugas lama yang pernah ia pegang.", en: "A member who is deleted, leaves a division, or becomes an intern is taken off that division's coordinator line automatically. Their name stays on the old tasks they handled." },
      { id: "Menghapus divisi selalu meminta konfirmasi: anggotanya dilepas dari divisi itu, koordinatornya ikut terhapus, dan tugas-tugasnya tetap ada di Work Breakdown tanpa divisi.", en: "Deleting a division always asks first: its members are taken out of it, its coordinator is removed, and its tasks stay in the Work Breakdown without a division." },
  ...
  },
  {
    ...
    steps: [
      ...
      { id: "Urutan baris bisa diubah dengan menyeret ikon titik-titik di ujung kiri baris, sama seperti di FAQ dan Hari-H. Urutannya tersimpan untuk semua orang, bukan hanya di layarmu.", en: "Rows can be reordered by dragging the grip icon at the far left of a row, the same as in the FAQ and Event-Day pages. The order is saved for everyone, not just on your screen." },
      ...
      { id: "Tab 'Compare' untuk membandingkan himpunan yang menerima ajakan. Klik 'Buat perbandingan', lalu pilih himpunan dari daftar yang berstatus DITERIMA di Reach & Offer. Satu himpunan tidak bisa dibuatkan dua kali.", en: "The 'Compare' tab compares the associations that accepted. Click 'Create comparison', then pick an association from those marked DITERIMA in Reach & Offer. An association can't be added twice." },
      { id: "Tiap perbandingan punya kartunya sendiri. Klik 'Tambah aspek' lalu isi Aspek Penilaian, Indikator yang Dinilai, Plus / Kelebihan, dan Minus / Kekurangan. Semuanya tersimpan otomatis. Tong sampah di kepala kartu menghapus perbandingan itu beserta seluruh penilaiannya.", en: "Each comparison gets its own card. Click 'Add aspect', then fill in the assessment aspect, the indicator, the plus points, and the minus points. Everything saves as you go. The bin icon in the card header removes that comparison and all its assessments." },
      { id: "Kalau sudah ada dua perbandingan atau lebih, muncul pilihan tampilan 'Kartu' dan 'Sanding'. 'Sanding' menaruh dua himpunan berdampingan, aspek demi aspek, supaya mudah membandingkan kelebihan dan kekurangannya langsung.", en: "Once there are two comparisons or more, a 'Cards' / 'Side by side' view switch appears. 'Side by side' puts two associations next to each other, aspect by aspect, to weigh their strengths and weaknesses directly." },
    ],
    tips: [
      ...
      { id: "Perbandingan yang sudah dibuat tetap ada meski status himpunannya di Reach & Offer berubah: hanya kamu yang bisa menghapusnya lewat tong sampah. Jadi penilaian tidak pernah hilang diam-diam.", en: "A comparison you have created stays even if the association's Reach & Offer response later changes: only you remove it, via the bin icon. So an assessment never disappears silently." },
    ],
    access: {
  ...
  },
  {
    ...
    steps: [
      ...
      { id: "Di bagian 'Tautan prospek', klik 'Tambah tautan' untuk melampirkan handbook, profil organisasi, atau proposal balasan dari himpunan itu. Boleh lebih dari satu tautan, dan tiap tautan bisa diberi nama serta dicentang 'Tampilkan juga di Super Link'. Kolom Catatan untuk catatan bebas.", en: "Under 'Prospect links', use 'Add link' to attach their handbook, org profile, or the proposal they sent back. You may attach more than one, and each link can be named and ticked as 'Also show in Super Link'. The Notes field is free text." },
      ...
      { id: "Filter Tahap memakai kotak centang, jadi beberapa tahap bisa dilihat sekaligus (misalnya 'diterima' dan 'ditolak' bersamaan). Tanpa centang berarti semua tahap tampil.", en: "The Stage filter uses checkboxes, so several stages can be shown at once (for example 'accepted' and 'rejected' together). No ticks means every stage is shown." },
    ],
    tips: [
      ...
      { id: "Tautan yang diterbitkan ke Super Link tetap dimiliki prospeknya: menyimpan ulang memperbaruinya, melepas centang atau menghapus tautannya ikut menghapus entri Super Link-nya. Satu alamat tidak boleh dipasang dua kali pada prospek yang sama.", en: "A link published to Super Link is still owned by its prospect: saving again updates it, and unticking or removing the link deletes its Super Link entry. The same address cannot be attached twice to one prospect." },
      { id: "Catatan yang panjang dipotong di tabel; klik 'Selengkapnya' untuk membaca isinya penuh tanpa membuka tombol Edit.", en: "A long note is clipped in the table; click 'Show more' to read all of it without opening the Edit dialog." },
      { id: "Mengganti nama himpunan di sini ikut mengganti nama kartu perbandingannya di menu Himpunan.", en: "Renaming an association here also renames its comparison card in the Himpunan menu." },
    ],
  ...
  },
  {
    ...
    tips: [
      { id: "Tautan berlabel 'Dari Work Breakdown' atau 'Dari Reach & Offer' terbit dari hasil tugas atau tautan prospek dan tetap tersambung dua arah: mengubah nama atau URL-nya di sini ikut mengubahnya di sumbernya. Divisi dan catatannya mengikuti sumbernya, jadi dikunci di sini.", en: "Links labelled 'From Work Breakdown' or 'From Reach & Offer' were published from a task result or a prospect link and stay connected both ways: changing the name or URL here changes it at the source too. Their division and note follow the source, so they are locked here." },
      { id: "Tautan 'Dari Work Breakdown' tidak bisa dihapus dari sini, karena hasil tugas selalu tampil di Super Link: hapus tautannya dari tugasnya di Work Breakdown, dan entrinya di sini ikut terhapus. Tautan 'Dari Reach & Offer' tetap bisa dihapus dari sini; centang 'Tampilkan juga di Super Link' di prospeknya ikut dicabut dan tautannya tetap ada di prospek itu.", en: "'From Work Breakdown' links cannot be deleted here, because task results always show in Super Link: remove the link from its task in Work Breakdown and the entry here goes with it. 'From Reach & Offer' links can still be deleted here; the prospect's 'Also show in Super Link' tick is cleared and the link stays on the prospect." },
      { id: "Tugas yang memakai tautan Super Link sebagai referensi selalu mengikuti URL dan judul terbarunya, jadi memperbaiki alamat di sini langsung memperbaiki semua tugas yang merujuknya.", en: "Tasks that use a Super Link entry as a reference always follow its current URL and title, so fixing an address here fixes every task that refers to it." },
  ...
  },
  {
    ...
    steps: [
      ...
      { id: "Satu Ormawa Visit punya satu RENCANA UTAMA, ditandai lencana di kartunya. Rencana utama inilah yang dipakai sebagai angka anggaran edisi: itu yang tampil di Dashboard, di kartu atas halaman ini, dan di daftar Ormawa Visit. Rencana lain adalah skenario pembanding dan tidak ikut dijumlahkan.", en: "Each Ormawa Visit has one MAIN PLAN, tagged with a badge on its card. That plan is the edition's budget figure: it is what the Dashboard, the stat card at the top of this page, and the Ormawa Visit list all report. The other plans are comparison scenarios and are not added to it." },
      { id: "Rencana pertama sebuah Ormawa Visit otomatis jadi rencana utama, jadi kalau kamu hanya punya satu rencana tidak ada yang perlu diatur. Begitu ada lebih dari satu, tombol bintang di kartu rencana memindahkan status utamanya. Hanya boleh ada satu rencana utama, jadi menunjuk yang baru otomatis melepas yang lama.", en: "The first plan of an Ormawa Visit becomes the main one automatically, so with a single plan there is nothing to set. Once there is more than one, the star button on a plan card moves the status. Only one plan can be the main one, so choosing a new one releases the old." },
      ...
      { id: "Ubah Nama item, Qty, Satuan, dan Harga satuan langsung di tabel; Total dan Subtotal dihitung otomatis. Tekan Enter untuk langsung menyimpan, atau klik di luar kolom.", en: "Edit the item name, Qty, Unit, and Unit price inline; Total and Subtotal are computed automatically. Press Enter to save immediately, or just click outside the field." },
      ...
      { id: "Seret ikon titik-titik di kiri baris untuk mengubah urutan item. Item juga bisa dipindah ke kategori lain: jatuhkan di baris judul kategori tujuan untuk menaruhnya di urutan teratas kategori itu, atau di antara item-itemnya untuk sekaligus menentukan posisinya.", en: "Drag the grip icon on the left of a row to reorder items. An item can also move to another category: drop it on that category's heading row to land at the top of it, or between its items to choose the position at the same time." },
    ],
    tips: [
      ...
      { id: "Kalau rencana utama dihapus, angka anggaran edisi otomatis jatuh ke rencana dengan total terbesar yang tersisa - Dashboard tidak akan pernah kosong karenanya. Kamu tetap bisa menunjuk rencana lain kapan saja.", en: "If the main plan is deleted, the edition's figure falls back to the largest plan left, so the Dashboard never goes blank because of it. You can still point to a different plan whenever you like." },
      ...
      { id: "Tekan Esc saat mengetik di kolom mana pun untuk membatalkan perubahan pada kolom itu.", en: "Press Esc while typing in any cell to discard that field's change." },
      { id: "Nama item tidak boleh dikosongkan: kalau kolomnya dikosongkan lalu ditinggalkan, nama yang tersimpan dikembalikan. Satuan boleh kosong, karena tidak semua pos punya satuan.", en: "An item name cannot be blank: empty the field and click away, and the stored name comes back. The unit may be blank, since not every line has one." },
      { id: "Item yang pindah kategori ikut mengambil warna kategori tujuannya, supaya satu kategori tidak pernah tampil dengan dua warna titik.", en: "An item that moves category adopts the destination category's colour, so one category never shows two different dots." },
    ],
  ...
  },
  {
    ...
    steps: [
      ...
      { id: "Saat membuat edisi baru, gunakan bagian 'Salin data dari Ormawa Visit lain' untuk menyalin Divisi, Anggota & Tim, Reach & Offer, Work Breakdown, Rundown, Job Hari-H, Super Link, atau Anggaran sebagai kerangka awal.", en: "When creating a new edition, use 'Copy data from another Ormawa Visit' to bring over Divisions, Members & Teams, Reach & Offer, Work Breakdown, Rundown, Event-Day Jobs, Super Link, or Budget as a starting point." },
      ...
      { id: "Salinan bisa dipersempit: untuk Anggota & Tim dan Work Breakdown, pilih divisi mana saja yang ikut; untuk Anggaran, pilih rencana RAB mana saja (kalau sumbernya punya lebih dari satu). Tanpa memilih, semuanya ikut tersalin.", en: "A copy can be narrowed: for Members & Teams and the Work Breakdown, pick which divisions come along; for Budget, pick which RAB plans (if the source has more than one). Pick none and all of it is copied." },
      { id: "Salin data juga tersedia saat MENGEDIT Ormawa Visit yang sudah ada - klik 'Salin data dari Ormawa Visit lain…' di dalam pop-up edit. Pilih dulu perlakuannya: 'Ganti total' menghapus data menu itu lebih dulu, atau 'Tambahkan' menyisakan data lama dan menaruh salinan di atasnya.", en: "Copying is also available when EDITING an existing Ormawa Visit - click 'Copy data from another Ormawa Visit…' inside the edit dialog. Choose how first: 'Replace entirely' deletes that menu's data first, or 'Add' keeps the existing data and puts the copy on top." },
      { id: "Sebelum menyalin ke edisi yang sudah ada, kamu harus mencentang konfirmasi yang menyebut dari Ormawa Visit mana data diambil dan Ormawa Visit mana yang akan ditimpa atau ditambahi - supaya tidak ada edisi yang tertimpa karena salah pilih.", en: "Before copying into an existing edition, you must tick a confirmation naming which Ormawa Visit the data comes from and which one will be overwritten or added to - so no edition gets clobbered by a wrong pick." },
    ...
    tips: [
      ...
      { id: "Menghapus sebuah Ormawa Visit menghapus SELURUH datanya: divisi, anggota, tugas, prospek, rundown, Job Hari-H, anggaran, dan Super Link-nya. Kalau hanya ingin membekukannya, pakai 'Kunci sebagai arsip'.", en: "Deleting an Ormawa Visit deletes ALL of its data: divisions, members, tasks, prospects, rundown, event-day jobs, budget and Super Link. To freeze it instead, use 'Lock as archive'." },
  ...
  },
  {
    ...
    tips: [
      ...
      { id: "Ubah nama tampilan dan foto profil lewat menu akun → 'Ubah Informasi Akun', atau tombol Ubah pada kartu Akun Saya di Pengaturan. Tersedia lima karakter foto profil, atau tetap memakai inisial namamu.", en: "Change your display name and profile picture from the account menu → 'Edit Account Details', or the Edit button on the My Account card in Settings. There are five character pictures to choose from, or keep your initials." },
      { id: "Email dan peran TIDAK bisa diubah dari sana. Peran diberikan admin lewat Role Request, dan email adalah kredensial login yang tidak bisa berganti tanpa konfirmasi.", en: "Email and role CANNOT be changed there. An admin grants the role through Role Request, and the email is a sign-in credential that cannot change without confirmation." },
      { id: "Tiga akun default (coordinator@, staff@, intern@) ditandai 'Akun bersama' dan kata sandinya sengaja TIDAK bisa diubah dari dalam aplikasi - menu 'Ubah Kata Sandi' tidak muncul untuk akun itu. Karena dipakai banyak orang, satu orang yang menggantinya akan mengunci semua yang lain. Kalau memang perlu dirotasi, admin melakukannya lewat SQL.", en: "The three default accounts (coordinator@, staff@, intern@) are marked 'Shared account' and their password deliberately CANNOT be changed from inside the app - the 'Change Password' entry does not appear for them. Because many people use them, one person changing it would lock everyone else out. If it genuinely needs rotating, an admin does it through SQL." },
      { id: "Akun yang mendaftar lewat Google tidak punya kata sandi di sistem ini, jadi tidak ada yang bisa diubah di sini. Kata sandinya adalah kata sandi akun Google-nya, dan diganti di halaman akun Google. Dialog 'Ubah Kata Sandi' mendeteksi hal ini dan mengatakannya, bukan menolak dengan pesan 'kata sandi lama salah'.", en: "An account that signed up with Google has no password in this system, so there is nothing here to change. Its password is the Google account password and is changed on Google's own account page. The 'Change Password' dialog detects this and says so, rather than failing with a misleading 'current password is wrong'." },
  ...
  },
  {
    ...
    purpose: {
      id: "Tanya jawab cepat soal sistem ini: menu, cara pakai, hak akses, dan data Ormawa Visit yang sedang dibuka, sampai ke rincian tiap barisnya.",
      en: "Quick answers about this system: menus, how to use them, access rules, and the data of the Ormawa Visit you are viewing, down to the detail of each row.",
    },
    steps: [
      ...
      { id: "Kamu bisa bertanya soal satu baris data tertentu, bukan hanya ringkasan. Misalnya 'kapan deadline tugas Susun konsep acara dan siapa PIC-nya', 'siapa MC sesi pembukaan', 'berapa harga air mineral di anggaran', 'siapa saja anggota divisi Event', atau 'tugas apa saja yang overtime'.", en: "You can ask about one specific row of data, not just a summary. For example 'when is the deadline for Susun konsep acara and who is its PIC', 'who is the MC for the opening session', 'how much is the mineral water in the budget', 'who is in the Event division', or 'which tasks are overtime'." },
    ...
    tips: [
      ...
      { id: "Rincian per baris tersedia untuk Ormawa Visit yang sedang kamu buka. Untuk edisi lain Violet tahu ringkasannya (jumlah tugas, divisi, prospek, anggaran, dan daftar namanya); kalau butuh rinciannya, ganti dulu edisinya lewat pemilih di bar atas.", en: "Row-by-row detail is available for the Ormawa Visit you currently have open. For other editions Violet knows the summary (how many tasks, divisions, prospects, the budget total, and the names); if you need the detail, switch edition first using the picker in the top bar." },
      { id: "Violet juga tahu cara kerja sistem ini, bukan hanya isinya: kolom apa saja yang disimpan tiap menu, kenapa sebuah tugas bisa jadi Overtime sendiri, apa bedanya tautan hasil dan tautan referensi, dan apa yang terjadi pada entri Super Link kalau sumbernya dihapus.", en: "Violet also knows how the system works, not only what is in it: which fields each menu stores, why a task can turn Overtime on its own, the difference between result links and reference links, and what happens to a Super Link entry when its source is deleted." },
      { id: "Datanya dibaca ulang setiap pertanyaan, jadi baris yang baru saja kamu simpan langsung bisa ditanyakan tanpa memuat ulang halaman.", en: "The data is re-read on every question, so a row you have just saved can be asked about immediately, without reloading the page." },
      ...
      { id: "Model AI-nya bisa diganti sendiri: klik nama model di bawah tulisan 'Violet' pada kepala panel, lalu pilih dari daftarnya. Ada dua model Google dan tiga model Groq, masing-masing dengan keterangan singkat kapan cocok dipakai. Pilihanmu diingat di perangkat ini saja, tidak ikut ke orang lain.", en: "You can change the AI model yourself: click the model name under 'Violet' in the panel header and pick from the list. There are two Google models and three Groq ones, each with a short note on when to reach for it. Your choice is remembered on this device only, not shared with anyone else." },
      { id: "Biarkan di 'Otomatis' kalau ragu: itu yang paling tahan gangguan, karena Violet boleh berpindah layanan sendiri saat satu layanan penuh. Model yang kamu pilih sendiri dipakai apa adanya, jadi kalau layanannya sedang bermasalah Violet melapor gagal alih-alih diam-diam menjawab pakai model lain.", en: "Leave it on 'Automatic' if unsure: it is the most resilient setting, because Violet may move between services by itself when one is exhausted. A model you pick yourself is used as-is, so if its service is having trouble Violet reports the failure instead of quietly answering with a different model." },
      { id: "Di bawah tiap jawaban tertulis model mana yang menulisnya, jadi kamu bisa membandingkan hasil dua model untuk pertanyaan yang sama.", en: "Under each answer it says which model wrote it, so you can compare two models on the same question." },
