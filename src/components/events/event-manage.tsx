    ...
    <EventFormDialog
      ...
      trigger={
        <DialogTrigger asChild>
          <Button>
            <Plus className="size-4" /> Tambah Ormawa Visit
          </Button>
        </DialogTrigger>
      }
    />
    ...
    <>
      ...
      <Dialog open={delOpen} onOpenChange={setDelOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Hapus Ormawa Visit?</DialogTitle>
            <DialogDescription>
              “{event.title}” beserta seluruh tugas, rundown, dan datanya akan dihapus permanen.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button variant="destructive" disabled={pending} onClick={() => start(async () => {
              ...
              if (res.ok) { toast.success("Ormawa Visit dihapus"); setDelOpen(false); } else toast.error(res.error);
