...
do $$ begin if to_regclass('public.task_comments') is not null then execute 'alter table public.task_comments disable row level security'; end if; end $$;
do $$ begin if to_regclass('public.broadcasts') is not null then execute 'alter table public.broadcasts disable row level security'; end if; end $$;
do $$ begin if to_regclass('public.broadcast_recipients') is not null then execute 'alter table public.broadcast_recipients disable row level security'; end if; end $$;
